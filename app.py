...
class NERModelCRF(nn.Module):
    def __init__(self, model_type, embedding_matrix, num_tags, pad_idx, max_len=64):
        ...
        self.spatial_dropout = SpatialDropout1D(0.15)
        hidden_size = 128
        ...
        self.extra_dense = nn.Linear(hidden_size*2, 64)
        self.relu = nn.ReLU()
        self.classifier = nn.Linear(64, num_tags)
        ...
        try:
            ...
            if st.button("Mulai Analisis Batch", type="primary"):
                ...
                for game_name, reviews in grouped_reviews.items():
                    st.markdown(f"### Analisis Game: {game_name}")
                    ...
                    with col_batch_kiri:
                        st.subheader(f"Detail Review ({game_name})")
